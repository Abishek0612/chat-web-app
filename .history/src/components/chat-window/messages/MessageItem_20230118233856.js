...
<TimeAgo
    ...
    />
        <IconBtnControl 
          ...
          {...(isLiked ? {color: 'red'} : {})}
        isVisible={canShowIcons} iconName='heart' tooltip='Like this message'  onClick={() => handleLike(message.id)} 
             badgeContent={likeCount}    />
             {
              isAuthor && <IconBtnControl 
              // eslint-disable-next-line no-constant-condition
            isVisible={canShowIcons} iconName='close' tooltip='Delete this message'  onClick={() => handleDelete(message.id)}  />
             }
      </div>
      <div>
